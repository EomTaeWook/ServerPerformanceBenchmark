...
public class ReqeustInfo : IRequestInfo<C2S>
{
    public C2S Body { get; set; }

    public string Key => string.Empty; // 프로토버프 통신에서는 Key 필요 없음, 빈 문자열

    public ReqeustInfo(C2S c2S)
    {
        Body = c2S;
    }
}
...
/// 수신 필터 클래스
/// </summary>
public class ReceiveFilter : FixedHeaderReceiveFilter<ReqeustInfo>
{
    /// <summary>
    ...
    protected override int GetBodyLengthFromHeader(byte[] header, int offset, int length)
    {
        ...
        var packetTotalSize = BitConverter.ToInt32(header, offset);
    ...
    protected override ReqeustInfo ResolveRequestInfo(ArraySegment<byte> header, byte[] buffer, int offset, int length)
    {
        var bodySize = BitConverter.ToInt32(header);

        var body = buffer.CloneRange(offset, bodySize);

        try
        {
            var proto = C2S.Parser.ParseFrom(body, 0, body.Length);
            return new ReqeustInfo(proto);
        }
        catch(Exception ex)
        {
